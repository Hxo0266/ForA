...
- **结构**：`0_inbox / 1_Syllabus / 2_Textbook&Lecture_PPT / 3_Essence / 4_Assignment / 5_Quiz / 6_Midterm Examination / 7_Final Examination / 10_Others / _System`
...
8. **中文输出走 UTF-8**。
9. **复查漂移（AIE-020）**：本 skill 是**一次性快照复制**，不是持续同步——母版每新增一条教训，已部署课程就多落后一版，而且**落后的副本只会显示「全绿」，不会报错**。所以：
   - 选 `--source` 时用**最近维护过的那门课**（当前为 `AIE2040`，它的 `_System/` 最新）；
   - 母版新增教训后，**逐个课程比对** `LESSON_CHECKS` 的键集合与 `ERROR_LOG.md` 的 ID 集合，缺失项按「**脚本实现 + ERROR_LOG 条目 + 变更记录**」三件套一起回补（只补文档等于没补）；
   - 各课程的编号是**独立命名空间**，「ID 对得上」不代表「内容对得上」——比对要看教训标题与实现，不能只看编号；
   - 回补脚本实现时，**按该课编号空间换名**（如母版 AIE-023 → MAT3007 的 AIE-024），别把母版编号直接塞进去。
