...
用途
    ...
    8_Correction_Notebook / 10_Others）。目录名一律从 _System/taxonomy.json 解析，
    代码不得硬编码（AIE-018/024）。
...
FALLBACK_TAXONOMY: dict = {
    "folders": {
        ...
        "correction": "8_Correction_Notebook",
        "others": "10_Others"
    },
    "optional_folders": {},
    "skip_names": [
...
def classify(path: Path, tax: dict, root: Path) -> dict:
    ...
    h_corr = hit_keywords(stem_tokens, text_all, kw.get("correction", []))
    lec_token = looks_like_lecture(stem_tokens, tax.get("lecture_token_patterns", []))

    if h_ans:
        flags.append("疑似含答案/解析：" + "/".join(h_ans))

    # ---- 定级：final > midterm > correction > quiz > assignment > (泛 exam) > syllabus
    ...
    if h_corr:
        return {"target": "correction", "reason": "文件名命中错题关键词：" + "/".join(h_corr), "flags": flags}
...
def main() -> int:
    ...
    # ---------- 报告 ----------
    mode = apply_mode(args.apply, len(moved))
    report = logs_dir / f"triage_{stamp}.md"
    report.write_text(
        render_report(root, tax_src, stamp, rows, skipped, limited, deferred, mode, fmap, tax),
        encoding="utf-8",
    )
    ...
    if args.apply:
        if moved:
            ...
            print(f"[triage] 本轮无可移动文件（Inbox 已空或全部被跳过），未写移动清单")
    else:
...
def apply_mode(applied: bool, moved_count: int) -> str:
    """报告/回执的「模式」标签必须忠实反映**真实动作**，而不是命令行参数。

    ⚠️ AIE-027：Inbox 已空时再跑 --apply（手滑重跑 / 自动化连跑两轮都会发生），
    `moved` 为 0。此时若仍标成「已实际移动」，就会给出一条指向不存在清单的
    --undo 指令，并让 AIE-005 误判「有 APPLY 却无清单」→ ❌ FAIL。
    """
    if not applied:
        return "DRY-RUN（仅出计划，未动文件）"
    return "APPLY（已实际移动）" if moved_count > 0 else "APPLY（无文件可移动）"


def render_report(root: Path, tax_src: str, stamp: str, rows, skipped, limited, deferred, mode: str, fmap: dict, tax: dict) -> str:
